...
每天 1-3 道题，需要用 JavaScript 或者 TypeScript
...
7. Extra Credit: `read` the **initial value** from `localStorage` as cached value at **_mounting phase(not at initial value of store)_** and **\*\***`write` the current value back to `localStorage` when component **_unmounting_**. So next time when you open the same application, value will be initialized as last change.
